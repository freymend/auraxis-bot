...
export async function button(interaction, locale, options) {
	...
	await medals(interaction, cName, platform, locale, true);
}
...
export async function execute(interaction, locale) {
	...
	await medals(interaction, cName, platform, locale);
}
 ...
 */
async function medals(interaction, cName, platform, locale='en-US', expanded=false){
	...
	if(remaining > 0){
		...
		await interaction.editReply({embeds: [resEmbed], compoents: []});
	}
}