...
export async function execute(interaction, locale="en-US"){
	...
	if(wInfo.maxIndirectDamage !== undefined && wInfo.directDamage !== undefined){ 
		if(wInfo.maxIndirectDamage != 0){ // Check for weapons like Tomoe and Doku scout rifles
			resEmbed.addFields(
				...
				{name: i18n.__({phrase: "Indirect Damage", locale: locale}), 
				value: `${wInfo.maxIndirectDamage} @ ${wInfo.maxIndirectDamageRadius}m\
	...
	wInfo.verticalRecoil != undefined && resEmbed.addFields({name: i18n.__({phrase: "Vertical Recoil", locale: locale}), value: `${wInfo.verticalRecoil}`, inline: true});
	...
	if(wInfo.useInWater !== undefined){
		if(wInfo.useInWater){
			resEmbed.addFields({name: i18n.__({phrase: "Usable in water", locale: locale}), value: i18n.__({phrase: "yes", locale: locale}), inline: true});
		}
		else{
			resEmbed.addFields({name: i18n.__({phrase: "Usable in water", locale: locale}), value: i18n.__({phrase: "no", locale: locale}), inline: true});
		}
	}
