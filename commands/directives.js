...
export async function button(interaction, locale, options) {
	...
	await directive(interaction, cName, platform, locale, true);
}
...
export async function execute(interaction, locale) {
	...
	await directive(interaction, cName, platform, locale);
}
 ...
 */
async function directive(interaction, cName, platform, locale="en-US", expanded=false,){
	...
	if(remaining > 0){
		...
		await interaction.editReply({embeds: [resEmbed]}, {components: []});
	}
}