 ...
 * @param {discord.Client} discordClient - discord client to use
 */
export async function check(discordClient){
	for(const server of servers){
		try{
			const now = new Date().toISOString();
			const territory = await territoryInfo(serverIDs[server]);
			const currentStatus = await query("SELECT * FROM openContinents WHERE world = $1;", [server]);
			await query("UPDATE openContinents SET indar = $1, hossin = $2, amerish = $3, esamir = $4, oshur = $5, koltyr = $6 WHERE world = $7;",
			...
			for(const cont of continents){
				if(territory[cont].locked != -1){
					await query("DELETE FROM bases WHERE continent = $1 AND world = $2;",
					[contIDs[cont], serverIDs[server]]);
				}
				else if(!currentStatus.rows[0][cont.toLowerCase()]){
					// If continent is open but recorded as closed
					try{
						const result = await query("SELECT u.channel, c.Indar, c.Hossin, c.Amerish, c.Esamir, c.Oshur, c.Koltyr, c.autoDelete\
					...
					update(discordClient, true); //Update trackers with new continent
				}
				if(territory[cont].locked == -1 != currentStatus.rows[0][cont.toLowerCase()]){
					// Update timestamp if there is a change
					await query(`UPDATE openContinents SET ${cont.toLowerCase()}change = $1 WHERE world = $2;`, [now, server]);
				}
			}
		}
		catch(err){
